...
; CHECK-NEXT:   EMIT vp<%2> = ALIAS-LANE-MASK ir<%b4>, ir<%c3> (write-after-read)
; CHECK-NEXT:   EMIT vp<%3> = popcount vp<%2>
; CHECK-NEXT:   EMIT vp<%4> = icmp ugt vp<%3>, ir<0>
; CHECK-NEXT:   EMIT vp<%5> = and vp<%4>, ir<%diff.check>
; CHECK-NEXT:   EMIT branch-on-cond vp<%5>
; CHECK-NEXT: Successor(s): ir-bb<scalar.ph>, vector.ph
; CHECK-EMPTY:
; CHECK-NEXT: vector.ph:
; CHECK-NEXT:   EMIT vp<%7> = broadcast ir<%b4>
; CHECK-NEXT:   EMIT vp<%8> = broadcast ir<%c3>
; CHECK-NEXT:   EMIT vp<%9> = ALIAS-LANE-MASK vp<%7>, vp<%8> (write-after-read)
; CHECK-NEXT:   EMIT vp<%popcount> = popcount vp<%9>
; CHECK-NEXT:   EMIT vp<%active.lane.mask.entry> = active lane mask ir<0>, ir<%wide.trip.count>, ir<1>
; CHECK-NEXT: Successor(s): vector.body
; CHECK-EMPTY:
; CHECK-NEXT: vector.body:
; CHECK-NEXT:   EMIT-SCALAR vp<%index> = phi [ ir<0>, vector.ph ], [ vp<%index.next>, vector.body ]
; CHECK-NEXT:   ACTIVE-LANE-MASK-PHI vp<%10> = phi vp<%active.lane.mask.entry>, vp<%active.lane.mask.next>
; CHECK-NEXT:   CLONE ir<%arrayidx> = getelementptr inbounds ir<%a>, vp<%index>
; CHECK-NEXT:   EMIT vp<%11> = and vp<%10>, vp<%2>
; CHECK-NEXT:   WIDEN ir<%1> = load ir<%arrayidx>, vp<%11>
; CHECK-NEXT:   CLONE ir<%arrayidx2> = getelementptr inbounds ir<%b>, vp<%index>
; CHECK-NEXT:   WIDEN ir<%2> = load ir<%arrayidx2>, vp<%11>
; CHECK-NEXT:   WIDEN ir<%add> = add ir<%2>, ir<%1>
; CHECK-NEXT:   CLONE ir<%arrayidx6> = getelementptr inbounds ir<%c>, vp<%index>
; CHECK-NEXT:   WIDEN store ir<%arrayidx6>, ir<%add>, vp<%11>
